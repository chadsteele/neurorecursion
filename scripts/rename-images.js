...
function extractImageMappings(dataContent, type) {
	...
	if (type === "conditions") {
		...
		for (const match of partnerMatches) {
			...
			mappings.set(filename, { id: filename.replace(/\.[^.]+$/, ""), imageUrl: filename, type: "partners" });
		}
	}
	...
	for (const file of files) {
		...
		if (newNameMap.has(file)) {
			...
			if (oldPath !== newPath) {
				...
				renamedFiles.push({ oldName: file, newName, dir: path.basename(dir) });
...
function updateDataFile(filePath, type, renamedFiles, cdnPath) {
	...
	const fileMap = new Map(renamedFiles.map(({ oldName, newName }) => [oldName, newName]));
		...
		content = content.replace(
			...
			(match, url) => {
				...
				if (fileMap.has(filename)) {
					const newFilename = fileMap.get(filename);
					const newUrl = url.replace(filename, newFilename);
					console.log(`  background_url: ${filename} → ${newFilename}`);
...
async function main() {
	...
	const conditionsMappings = extractImageMappings(conditionsContent, "conditions");
	const renamedConditions = renameFilesInDirectory(BACKGROUNDS_DIR, conditionsMappings);
	...
	const renamedPioneers = renameFilesInDirectory(PIONEERS_DIR, pioneersMappings);
	...
	const renamedPartners = renameFilesInDirectory(PARTNERS_DIR, partnersMappings);
