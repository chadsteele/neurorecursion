...
function extractAllIDs() {
	...
	const partnerMatches = ngosContent.matchAll(/imageSrc:\s*"\/partners\/([^"]+)"/g);
...
function verifyImageNaming() {
	...
	console.log(`  ✓ Correct: ${results.conditions.correct}/${results.conditions.total}`);
	if (results.conditions.incorrect.length > 0) {
		console.log(`  ✗ Incorrect: ${results.conditions.incorrect.slice(0, 5).join(", ")}`);
	}
	...
	console.log(`  ✓ Correct: ${results.pioneers.correct}/${results.pioneers.total}`);
	if (results.pioneers.incorrect.length > 0) {
		console.log(`  ✗ Incorrect: ${results.pioneers.incorrect.slice(0, 5).join(", ")}`);
	}
	...
	console.log(`  ✓ Correct: ${results.partners.correct}/${results.partners.total}`);
	if (results.partners.incorrect.length > 0) {
		console.log(`  ✗ Incorrect: ${results.partners.incorrect.slice(0, 5).join(", ")}`);
	}
	...
	const totalFiles =
		results.conditions.total + results.pioneers.total + results.partners.total;
	const totalCorrect =
		results.conditions.correct + results.pioneers.correct + results.partners.correct;
	console.log(`\nTotal: ${totalCorrect}/${totalFiles} files correctly named`);
	console.log(`  • Conditions: ${results.conditions.correct}/${results.conditions.total}`);
	console.log(`  • Pioneers: ${results.pioneers.correct}/${results.pioneers.total}`);
	console.log(`  • Partners: ${results.partners.correct}/${results.partners.total}`);
