<script>
	...
	const EMAIL_COOLDOWN_STORAGE_KEY = "auth:login-email-cooldown-until"
	...
	async function handleFormSubmit(event) {
		...
		if (cooldownRemaining > 0) {
			errors.submit = `Please wait ${cooldownRemaining}s before requesting another sign-in link.`
			return
		}
		...
		try {
			...
			if (error) {
				if (isRateLimitError(error.message)) {
					...
					errors.submit =
						error.message || "Failed to send sign-in link"
				}
				isSubmitting = false
				return
			}
			...
			successMessage =
				"Check your email for a secure sign-in link. Open it on this device to continue."
			startCooldown(EMAIL_COOLDOWN_SECONDS)
			formData = {email: ""}
			isSubmitting = false
		} catch (err) {
...
<div class="login-container">
	<div class="login-card">
		...
		<h1>Welcome Back</h1>
		<p class="subtitle">Sign in with a secure email link</p>
